...
namespace glmath {
	...
	mat4 operator*(const mat4 &left, const mat4 &right) {
		...
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				result.m[(4 * i) + j] = left.m[(4 * i)] * right.m[j] + 
										left.m[(4 * i) + 1] * right.m[(4 * 1) + j] + 
										left.m[(4 * i) + 2] * right.m[(4 * 2) + j] +	
										left.m[(4 * i) + 3] * right.m[(4 * 3) + j];
	...
	std::ostream & operator<<(std::ostream &left, const mat4 &right){
		left << std::dec;
		left << char(218) << ' ' << centered(right.m00, NUM_LEN) << '\t' << centered(right.m01, NUM_LEN) << '\t' << centered(right.m02, NUM_LEN) << '\t' << centered(right.m03, NUM_LEN) << ' ' << char(191) << std::endl;
		left << char(179) << ' ' << centered(right.m10, NUM_LEN) << '\t' << centered(right.m11, NUM_LEN) << '\t' << centered(right.m12, NUM_LEN) << '\t' << centered(right.m13, NUM_LEN) << ' ' << char(179) << std::endl;
		left << char(179) << ' ' << centered(right.m20, NUM_LEN) << '\t' << centered(right.m21, NUM_LEN) << '\t' << centered(right.m22, NUM_LEN) << '\t' << centered(right.m23, NUM_LEN) << ' ' << char(179) << std::endl;
		left << char(192) << ' ' << centered(right.m30, NUM_LEN) << '\t' << centered(right.m31, NUM_LEN) << '\t' << centered(right.m32, NUM_LEN) << '\t' << centered(right.m33, NUM_LEN) << ' ' << char(217) << std::endl;
		return left;
	}
